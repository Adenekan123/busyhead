...
Router.post('/api/drafts/add', isLoggedIn, async (req, res) => {
    ...
    try {
        let deletedCount = 0;
        for(let i = 0; i < allSelects.length; i++) {
            deletedCount += await moveTasks(Draft,Todo,allSelects[i].selectid,allSelects[i].taskids,req.user._id);
        }
        if(deletedCount > 0){
...
Router.post('/api/drafts/update', isLoggedIn,async (req, res) => {
    const {draftid,id,title,description,date} = req.body;
    try {
        const response = await updateTask(Draft,draftid,id,{title,description,date},req.user._id)
